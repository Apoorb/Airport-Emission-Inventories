...
if __name__ == "__main__":
    path_nfdc = os.path.join(PATH_RAW, "madhu_files", "FAA_NFDC_Facilities.csv")
    ...
    path_counties_shp = os.path.join(
        PATH_RAW, "Texas_County_Boundaries_Detailed-shp", "County.shp"
    )
    counties_tx = gpd.read_file(path_counties_shp)
    assert airport.crs == counties_tx.crs, (
        "Make sure the coordinate reference system is same between the "
        "geodataframes before joining."
    )
    counties_tx_1 = (
        ...
        .rename(
            columns={
                "cnty_nm": "county",
                "dist_nm": "district_tx_boundar",
                "cnty_fips": "fips_tx_boundar",
            }
        )
        .assign(
            county=lambda df: (df.county.str.strip().str.lower().str.replace("'", ""))
        )
        .filter(items=["county", "district_tx_boundar", "fips_tx_boundar", "geometry"])
    )
    ...
    airport_tx_fil_counties = (
        gpd.sjoin(airport_tx, counties_tx_1, how="left", op="intersects")
        .rename(
            columns={
                "ident": "facility_id",
                "name": "facility_name",
    ...
    nfdc_arpt_mrg["qc"] = (
        nfdc_arpt_mrg.county_arpt == nfdc_arpt_mrg.county_nfdc
    ).astype(int)
    ...
    counties_tx_1.drop("geometry", axis=1).to_csv(path_out_tx_counties)
    ...
    assert all(
        nfdc_arpt_mrg.loc[~mask, "county_nfdc"]
        == nfdc_arpt_mrg.loc[~mask, "county_arpt"]
    )
