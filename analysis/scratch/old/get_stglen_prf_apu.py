...
if __name__ == "__main__":
    ...
    gse_ac_def = pd.read_sql("SELECT * FROM [dbo].[FLT_GSE_AC_DEFAULTS]", conn)
    anp_arp_prof_1 = anp_arp_prof.rename(columns=get_snake_case_dict(anp_arp_prof))
    ...
    gse_ac_def_1 = gse_ac_def.rename(columns=get_snake_case_dict(gse_ac_def)).filter(
        items=["airframe_id", "def_op_time_dep", "def_op_time_arr"]
    )
    def_apu_2 = def_apu_1.merge(apu_name_1, on="apu_id").filter(
        items=["apu_id", "apu_name", "airframe_id", "user_defined"]
    )

    fleetmix_dfw_2 = fleetmix_dfw_1.merge(
        anp_arp_prof_1, left_on="anp_airplane_id", right_on="acft_id", how="left"
    )

    fleetmix_dfw_3 = fleetmix_dfw_2.merge(def_apu_2, on="airframe_id", how="left")
