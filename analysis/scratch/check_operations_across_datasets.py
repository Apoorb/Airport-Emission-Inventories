...
if __name__ == "__main__":
    ...
    path_tfmsc2020_mv = os.path.join(
        PATH_RAW, "madhu_files", "FAA_2020TFMSC.csv"
    )
    path_taf = os.path.join(
        PATH_RAW, "madhu_files", "TX_TAF1990_2045.csv"
    )
    path_airnav = os.path.join(
        PATH_RAW, "madhu_files", "AirNav.csv"
    )
    ...
    nfdc_facilities_may21_1_non_zero = (
            nfdc_facilities_may21_1
            .loc[lambda df: df.total_ops > 0]
            .assign(
                facility_id=lambda df: df.facility_id.str.replace("'", "")
            )
    )
    nfdc_facilities_may21_1_commercial = (
        nfdc_facilities_may21_1
        .loc[lambda df: ~ df.operations_commercial.isna()]
    )
    ...
    tfmsc2020_1 = (
        tfmsc2020
        .rename(columns=get_snake_case_dict(tfmsc2020))
    ...
    tfmsc2020_1_facilites = (
        tfmsc2020_1
        .filter(items=["facility_id", "airport"])
        .drop_duplicates("facility_id")
    )

    taf_1 = (
        taf
        .rename(columns=get_snake_case_dict(taf))
        .rename(columns={"loc_id": "facility_id", "aport_name": "airport"})
    )
    ...
    airnav_1 = (
        airnav
        .rename(columns=get_snake_case_dict(airnav))
        .assign(
            annualized_aircraft_operations=lambda df: df.annualized_aircraft_operations.astype(float)
        )
