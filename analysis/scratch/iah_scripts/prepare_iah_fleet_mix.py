...
if __name__ == "__main__":
    ...
    assert np.isclose(fleetmix_iah_1.fleetmix.sum(), 1), (
    ...
    fleetmix_iah_eng_arfm = fleetmix_iah_1.merge(
        eng_df_1, on="engine_id", how="left").merge(
        airfm_df_1,
    ...
    anp_arp_prof = pd.read_sql(
        "SELECT * FROM [dbo].[FLT_ANP_AIRPLANE_PROFILES]",
        conn)
    anp_arp_prof_1 = (
        anp_arp_prof
    ...
    anp_arp_prof_stnd = (
        anp_arp_prof_1
        .loc[lambda df: df.prof_id1 == "STANDARD"]
    )
    ...
    anp_arp_prof_non_stnd = anp_arp_prof_1.loc[lambda df: df.acft_id.isin(
        missing_ids)]
    assert anp_arp_prof_non_stnd.prof_id1.unique() == ["NOISEMAP"], (
        "All non-standard profiles are NOISEMAP profiles."
    )
    anp_arp_prof_fin = (
        pd.concat(
            [anp_arp_prof_stnd, anp_arp_prof_non_stnd])
    ...
    anp_heli_prof = pd.read_sql(
        "SELECT * FROM [dbo].[FLT_ANP_HELICOPTER_PROFILES]",
        conn
    )
    anp_heli_prof_1 = (
        anp_heli_prof
        .rename(columns=get_snake_case_dict(anp_heli_prof))
        .loc[lambda df: df.op_type.isin(["A", "D"])]
    )
    anp_heli_prof_stnd = (
        anp_heli_prof_1
    ...
    assert len(anp_heli_prof_stnd) == len(anp_heli_prof_1), (
        "Some helicopters have non standard profile.")
    anp_heli_prof_fin = (
        anp_heli_prof_stnd
        .rename(columns={"helo_id": "acft_id"})
        .sort_values(["acft_id", "op_type", "profile_id"])
        .drop_duplicates(["acft_id", "op_type"])
        .rename(columns={"prof_id1": "profile", "prof_id2": "stage_len"})
        .filter(
            items=["acft_id", "op_type", "profile", "stage_len", "weight"])
        .reset_index(drop=True)
    )

    anp_arp_heli_prof_fin = (
        pd.concat([anp_arp_prof_fin, anp_heli_prof_fin])
    )
    ...
    assert def_apu_2.duplicated("airframe_id").sum() == 0, (
        "Duplicated airframes!")
    conn.close()

    # 5 Add APU name to fleetmix.
    fleetmix_iah_eng_arfm_apu = (
        fleetmix_iah_eng_arfm
        .merge(
            def_apu_2,
            on=["airframe_id"],
            how="left"
    ...
    fleetmix_iah_eng_arfm_apu_fil = (
        fleetmix_iah_eng_arfm_apu.loc[
            lambda df: ~ df.anp_airplane_id.isin(missing_profiles)]
        .assign(
    ...
    assert fleetmix_iah_eng_arfm_apu_prf.stage_len.isna().sum() == 0, (
    ...
    fleetmix_iah_eng_arfm_apu_prf["op_counts"] = \
        ...
        "airframe_id", "engine_id"]).op_type.transform("count")

    assert all((fleetmix_iah_eng_arfm_apu_prf.groupby([
        "airframe_id", "engine_id"]).op_type.count() == 2).values), (
    ...
    tracks_iah_1 = (
        tracks_iah
        .rename(columns=get_snake_case_dict(tracks_iah))
        .filter(items=["aircraft_type", "track_id", "rwy_end_id", "track_name", "op_type"])
        .groupby(["op_type", "aircraft_type"]).apply(lambda df: df[df.track_id ==
                                                df.track_id.min()])
        .reset_index(drop=True)
    )
    subtracks_iah_1 = (
        subtracks_iah.rename(columns=get_snake_case_dict(subtracks_iah))
        .filter(items=["subtrack_id", "track_id", "subtrack_num"])
    )
    segments_iah_1 = (
        segments_iah
        .rename(columns=get_snake_case_dict(segments_iah))
        .filter(items=["segment_id", "subtrack_id", "segment_num",
                       "segment_type", "param_1", "param_2"])
        .rename(columns={"param_1": "dist_or_rad", "param_2": "turn_angle"})
    )
    runway_iah_1 = (
        runway_iah
        .rename(columns=get_snake_case_dict(runway_iah))
        .filter(items=["rwy_end_id", "rwy_end_name"])
    )

    tracks_iah_2 = (
        tracks_iah_1
        .merge(runway_iah_1, on="rwy_end_id", how="left")
    ...
    arpt_layout_1 = (
        arpt_layout
        .rename(columns=get_snake_case_dict(arpt_layout))
    )
