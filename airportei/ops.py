...
def get_nfdc_ops(path_nfdc_may21_: str) -> dict[str, dict[str, list]]:
    ...
    nfdc_facilities_may21_1 = (
        ...
        .assign(
                ...
                effective_yr=lambda df: (
                    df.effective_date.str.split("/", expand=True)[2]),
                total_ops=lambda df: (
    ...
    oth_serv_2d_list = list(map(lambda ls: ls.split(","),
             list(filter(lambda x: x == x,
                         nfdc_facilities_may21_1.other_services.unique()))
             ))
    ...
    return {"nfdc_facilities_may21_1": nfdc_facilities_may21_1,
...
if __name__ == "__main__":
    ...
    path_tfmsc_19 = os.path.join(PATH_RAW, "madhu_files",
                                 "FAA_2019TFMSC.csv")
    ...
    tfmsc_19_df_1 = (
        tfmsc_19_df
        .rename(columns=get_snake_case_dict(tfmsc_19_df))
    ...
    tfmsc_19_df_2 = (
        ...
        .assign(
            ...
            aircraft_type_id=lambda df: (df.aircraft_type_id.replace(
                    ["Myst�re", "A�rospatiale", "Br�guet", "#NAME?"],
                    ["Mystere", "Aerospatiale", "Breguet", -999],
                    regex=True,
            )),
            aircraft_type=lambda df: (df.aircraft_type.replace(
    ...
    fill_engine_type_df = (
        tfmsc_19_df_2[["aircraft_id", "aircraft_type", "aircraft_type_id",
                       "engine_type"]]
        ...
        .assign(
            ...
            aircraft_id_and_type=lambda df: df.aircraft_id + " "
                                            + df.aircraft_type,
    ...
    aedt_3d_airframe_1 = (
        aedt_3d_airframe
        .rename(columns=get_snake_case_dict(aedt_3d_airframe))
    ...
    tfmsc_19_df_2_has_aircraft_id = (
        tfmsc_19_df_2
        .loc[lambda df: df.aircraft_id != -999]
    )
    tfmsc_19_df_2_has_aircraft_id_no_engine_type = (
        tfmsc_19_df_2_has_aircraft_id
        .loc[lambda df: df.engine_type.isnull()]
    )
    ...
    tfmsc_19_df_2_no_aircraft_id_facilities_valid_data = (
        tfmsc_19_df_2
        .loc[lambda df: (df.facility_id.isin(no_aircraft_id_facilities))
                        & (df.aircraft_id != -999)]
    )
    ...
    tfmsc_19_df_2_no_aircraft_id_fill_missing = (
        tfmsc_19_df_2_no_aircraft_id_fill
        .loc[lambda df: df.aircraft_type_fill.isna()]
    )


    len(tfmsc_19_df_2_no_aircraft_id_facilities_valid_data.facility_id.unique())

    tfmsc_19_df_1_bad_names = (
        tfmsc_19_df_1
        .loc[lambda df: df.aircraft_type_id.str.contains(
            r'[^-\w /,;()<`]'
           )]
    )

    assert all(
        tfmsc_19_df_2.aircraft_type_id.str.contains(
            r'[-\w /,;()<`]'
           )
